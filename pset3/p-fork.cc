...
void process_main() {
  ...
  if (p2 == 0) {
    ...
    assert(p2 != p1);
    assert(p2 != intermediate_pid);
    assert(p2 != initial_pid);
    assert(final_pid == intermediate_pid);
  }
  /*
USER PANIC: p-fork.cc:42: user assertion 'p2 != p1 && p2 != intermediate_pid &&
                                      p2 != initial_pid' failed
    */
  ...
  while (heap_top != stack_bottom) {
    ...
    if (x < p) {
      ...
      for (unsigned long *l = (unsigned long *)heap_top;
           l != (unsigned long *)(heap_top + PAGESIZE); ++l) {
        assert(*l == 0);
      }
      ...
      *heap_top = p;
      ...
      uintptr_t addr = rand((uintptr_t)heap_bottom, (uintptr_t)heap_top - 1);
      *((char *)addr) = p;
    }
    sys_yield();
  }
