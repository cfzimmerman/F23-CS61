...
struct io61_file {
    ...
    // offset of first character in `cbuf`
    off_t tag;
    // next offset to read or write (non-positioned mode)
    off_t pos_tag;
    // offset one past last valid character in `cbuf`
    off_t end_tag;


    // Positioned mode
    // has cache been written?
    std::atomic_bool dirty = false;
    // is cache in positioned mode?
    bool positioned = false;

    // The lock controlling access to the cache
    std::recursive_mutex cache_lock;
    // The lock controlling access to file ranges
    std::unique_ptr<range_locks> ranges = nullptr;
};
...
io61_file* io61_fdopen(int fd, int mode) {
    ...
    if (f->seekable && filesize > 0) {
        f->ranges = std::make_unique<range_locks>(filesize);
    }
    return f;
}
...
int io61_readc(io61_file* f) {
    assert(!f->positioned);
    std::unique_lock lk(f->cache_lock);
...
int io61_try_lock(io61_file* f, off_t start, off_t len, int locktype) {
    ...
    return f->ranges->try_lock_range(start, len, locktype);
}
...
int io61_lock(io61_file* f, off_t start, off_t len, int locktype) {
    ...
    f->ranges->lock_range(start, len, locktype);
    return 0;
}
...
int io61_unlock(io61_file* f, off_t start, off_t len) {
    ...
    f->ranges->unlock_range(start, len);
    return 0;
}
